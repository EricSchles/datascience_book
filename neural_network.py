...
class Dense:
    def __init__(self, num_inputs, num_outputs, activation_function):
        ...
        self.synapses = np.random.uniform(-1, 1, (num_inputs, num_outputs)) 
    ...
    def forward(self, X):
        self.output = self.activation_function(X.dot(self.synapses))
        return self.output
    ...
    def update_weights(self, X, learning_rate):
        self.synapses += X.T.dot(self.delta) * learning_rate
